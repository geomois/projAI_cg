...
class Models:
	...
	def applymodel(self,id,opt,loss,x_train,x_test,y_train,y_test):
		if id==1:
			autoencoder=model1()
		if id==2:
			autoencoder=model2()
	...
	def model2use(self,seclength):
		#having extra channel for images. works.
		input_img = Input(shape=(2, 1, seclength))
		x = Convolution2D(32, 1, 32, activation='relu', border_mode='same')(input_img)#seclengthx32
		x = MaxPooling2D((1,3))(x)#seclength/2x32
		x = Convolution2D(16,1,16,activation='relu',border_mode='same')(x)#seclength/2x16
		x = MaxPooling2D((1,3))(x)#seclength/4x16
		x = Convolution2D(16,1,16,activation='relu',border_mode='same')(x)#seclength/4x16
		x = UpSampling2D((1,3))(x)#seclength/2x16
		x = Convolution2D(32, 1, 32, activation='sigmoid', border_mode='same')(x)#seclength/2x32
		x = UpSampling2D((1,3))(x)#seclengthx32
		x = Convolution2D(4,1,16,activation='sigmoid', border_mode='same')(x)#seclengthx4
		decoded = Convolution2D(1,1,8,activation='sigmoid', border_mode='same')(x)#seclength
		autoencoder = Model(input_img, decoded)
		return autoencoder

	def applymodel(self,autoencoder,x,y,pct,opt,loss,epochs,batch,name):
...
"""
x = np.random.random((3000,2,1,22050))
y = np.random.random((3000,1,1,22050))
print "size ",x.shape,"y.shape"

mymodel=Models()
autoencoder=mymodel.model2use(22050)#pooling 3 
mymodel.applymodel(autoencoder,x,y,0.3,'adadelta','bin',15,128,'keras_w')
	
trainedautoencoder=mymodel.model2use(22050)
