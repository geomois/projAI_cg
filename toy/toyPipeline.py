...
class MyAudio:
	...
	def __init__(self,audio_sample_freq,audio""",filename,maxFreq,"""chunk_duration):
...
"""
	...
	def split(self""",write_files"""):
		...
		self.chunks2=[]
        	...
        	for start_pos in range(0,self.audio.shape[0]"""self.downsampled_audio.shape[0]""",samples_in_chunk):
                	end_pos = start_pos + samples_in_chunk
                	if end_pos >= self.audio.shape[0]"""len(self.downsampled_audio)""":
                        	end_pos = self.audio.shape[0]"""len(self.downsampled_audio)""" - 1
               		#print "start_pos, end_pos:",start_pos, " ",end_pos
                	chunk = """self.downsampled_audio"""self.audio[start_pos : end_pos,0]
...
test=MyAudio(downRate,signal,1)
